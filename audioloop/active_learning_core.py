...
def run_binary_inference(
    ...
    # Create temporary CSV with filtered dataset entries
    # Using temp CSV instead of in-memory data for simpler API and better debuggability
    temp_csv = None
    try:
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False, newline="") as f:
            temp_csv = Path(f.name)
            writer = csv.DictWriter(
                f, fieldnames=["filename", "label", "original_class", "audio_path"]
            )
            writer.writeheader()
            ...
            for item in metadata:
                ...
                is_positive = dataset_config.get_binary_label(
                    item, positive_class_id, positive_class_name
                )
