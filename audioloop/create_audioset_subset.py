...
def create_subset(class_name: str, total_samples: int, positive_ratio: float = 0.05):
    ...
    print(f"Saved {len(subset):,} samples ({len(selected_pos)} pos, {len(selected_neg)} neg) to {output_path}")
    print(f"Next: python -m audioloop.create_all_specs --dataset audioset --metadata-file {output_path}")
...
def main():
    ...
    parser.add_argument("positive_ratio", type=float, nargs="?", default=0.05, help="Positive ratio (default: 0.05)")
