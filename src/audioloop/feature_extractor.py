...
(`_load_audio`), and cache-path resolution (`get_cached_feature_path`). Each extractor's
cached features live in their own subdir (`cache_subdir`) under the shared cache root, so
features from different extractors coexist without colliding; `ensure_cache_dir` stamps a
validating `extractor.json` manifest into that subdir. Concrete extractors override only
