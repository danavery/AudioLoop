...
class TestAutoLabelingUtility:
    ...
    def create_test_candidates_csv(self, candidates_data, include_ground_truth=True):
        ...
        if include_ground_truth:
            fieldnames = [
                ...
                'prediction', 'entropy', 'prob_negative', 'prob_positive',
                'true_is_positive', 'correct', 'original_class', 'fold', 'filepath'
            ]
        else:
    ...
    def test_auto_label_mixed_boolean_formats(self):
        ...
        candidates_data = [
            {'filename': 'test1.wav', 'true_is_positive': 'true'},   # lowercase
            {'filename': 'test2.wav', 'true_is_positive': 'TRUE'},   # uppercase  
            {'filename': 'test3.wav', 'true_is_positive': 'True'},   # mixed case
            {'filename': 'test4.wav', 'true_is_positive': 'false'},  # lowercase false
            {'filename': 'test5.wav', 'true_is_positive': ''},       # empty string
        ]
